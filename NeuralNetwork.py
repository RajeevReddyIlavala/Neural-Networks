...
class NeuralNetwork:
	...
	def back_prop(self,net_inputs,observed_out,target_out):
		...
		for i in range(self.no_of_layers-2, 0, -1):
			s = (self.weights[i]@sensitivities[0]) * sigmoid_prime(net_inputs[i])
	...
	def train_NN(self, x_in, target_out, lr):
		...
		for l in range(0,self.no_of_layers-1):
			self.weights[l] = self.weights[l] -  ((lr/batch_size) * (activations[l]@np.transpose(sensitivities[l])))
	...
	def SGD(self,input_patterns,target_out,lr,epochs,batch_size,tv_x=None,tv_y=None):
		...
		return training_cost,train_accuracy	
...
def main():
	...
	training_cost,tv_cost=NN.SGD(tr_x,tr_y,0.5,100,50)
	...
	accuracy=NN.accuracy(tr_x,tr_y)
